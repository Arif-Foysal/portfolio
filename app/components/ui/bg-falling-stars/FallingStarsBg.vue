...
function drawStar(star: Star) {
  ...
  // Draw blurred trail (longer, with higher opacity)
  ctx.save(); // Save current context state for restoring later
  ctx.strokeStyle = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.4)`; // Increased from 0.2
  ctx.lineWidth = size * 3; // Increased from 2.5
  ctx.shadowBlur = 45; // Increased from 35
  ctx.shadowColor = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 1)`; // Increased from 0.8
  ctx.beginPath();
  ctx.moveTo(x2d, y2d);
  ...
  ctx.strokeStyle = `rgba(${rgb.r}, ${rgb.g}, ${rgb.b}, 0.8)`; // Increased from 0.6
  ctx.lineWidth = size * 1.5; // Increased size
  ctx.beginPath();
  ctx.moveTo(x2d, y2d);
  ...
  ctx.beginPath();
  ctx.arc(x2d, y2d, size / 2, 0, Math.PI * 2); // Increased dot size from size/4 to size/2
  ctx.fill();
